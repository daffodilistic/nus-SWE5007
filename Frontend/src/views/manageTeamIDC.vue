          ...
          <tbody v-for="(team, index) in paginatedTeams" :key="index">
            <tr :class="{'parent-row': true, 'active-row': activeRow === index}" @click="toggleRow(index)">
              ...
              <td>
                ...
                <!-- View Icon -->
                <b-button @click="viewScore(team.id)" id="viewScore" variant="outline-primary" class="delete-button"  v-b-tooltip.hover="'Click to view team score'">
...
import { DELETE_IDC_TEAM_BASE_URL, ADD_MEMBER_IDC_TEAM_BASE_URL,UPDATE_IDC_TEAM_BASE_URL,VIEW_IDC_TEAM_BASE_URL, GET_ALL_USER_INFO_BASE_URL,GET_ALL_IDC_TEAM_BASE_URL,GET_ALL_GAME_TEAM_BASE_URL,GET_ALL_GAME_GROUP_BASE_URL} from '@/api';
  ...
  methods: {
    ...
    async fetchUsers(teamId,teamName) {
      ...
      try {
        ...
        this.userList = allUsers.filter((record) => record.hasOwnProperty('idcTeam'));
      ...
      async editTeam(index) {
        ...
        if (team.editing) {
          ...
          try {
            ...
            const response = await axios.put(`${UPDATE_IDC_TEAM_BASE_URL}`, requestBody, { headers });
