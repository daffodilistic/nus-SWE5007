<template>
    ...
    <form>
      <div class="form-row">
        <div class="form-row">
          <label for="username"><i class="fas fa-user" style='color: rgb(65, 127, 202)'></i> Username</label>
          <input type="text" id="username" name="username" v-model="input.username" placeholder="Username" />
        </div>
        <br>
        <div class="form-row">
          <label for="password"><i class="fas fa-lock" style='color: rgb(65, 127, 202)'></i> Password</label>
...
input[type="submit"] {
    ...
    padding: 10px 0; /* Remove fixed padding and use vertical padding */
...
.background-container {
  flex: 1;
  /* Take up the left portion of the screen */
  background: url('@/assets/BG2.jpg') no-repeat center center fixed;
  background-size: cover;
  background: rgba(0, 0, 0, 0.5) url('../assets/BG2.jpg');
  width: 50%;
  /* Set a width for the background container */
}

.form-container {
  flex: 1;
  /* Take up the right portion of the screen */
  display: flex;
